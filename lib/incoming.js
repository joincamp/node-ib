...
function UnderrunError() {
  //console.log('Underrun error!', arguments);
}

Incoming.prototype.dequeue = function () {
  if (this._dataQueue.length == 0) throw new UnderrunError();
...
Incoming.prototype.process = function () {
  ...
  while (true) {
    //console.log(this._dataQueue);
    dataQueueSnapshot = this._dataQueue.slice();
    //console.log('dataQueueSnapshot: ', dataQueueSnapshot);

    try {
      //clear the Emit Queue (added by Heber...if this doesn't get cleared, it piles up whenever there's an error)
      this._emitQueue = [];
